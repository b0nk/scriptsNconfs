...
bb_codes = [line.strip() for line in open('bitbin/codes.txt', 'r')]
...
def reset():
  global proxyList, proxy_srcs, userAgents, bb_codes
  proxy_srcs = [line.strip() for line in open('psrc.txt', 'r')]
  userAgents = [line.strip() for line in open('ua.txt', 'r')]
  bb_codes = [line.strip() for line in open('bitbin/codes.txt', 'r')]
  proxyList = []
...
def openLink(h, bc, headers):
  try:
    link = 'http://bitbin.it/%s'
    myprint(3, "Connecting: %s ..." % link % (bc))
    r2, cont = h.request(link % (bc), "GET", headers = headers)
    if r2:
      if r2.status is 200:
        myprint(1, "#######################SUCCESS!#######################")
        return True
...
def visitLinks():
  global userAgents, proxyList, bb_codes
  random.shuffle(userAgents)
  random.shuffle(bb_codes)
  ...
  while proxyList:
    try:
      ...
      bb_code = random.choice(bb_codes)
      proc = pool.apply_async(func = openLink, args = (h2, bb_code, headers))
      res = proc.get(timeout = 10)
      if res:
        win += 1
